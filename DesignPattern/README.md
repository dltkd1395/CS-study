...
- 이터레이터 패턴을 사용하면 모든 항목에 일일이 접근하는 작업을 컬렉션 객체가 아닌 반복자 객체에서 맡게된다.
- 이렇게 하면 집합체의 인터페이스 및 구현이 간단해질 뿐 아니라, 집합체에서는 반복작업에서 손을 떼고 원래 자신이 할 일 (객체 컬렉션 관리)에만 전념 할 수 있다.

### 예제
두 개의 서로 다른 식당이 있고 각각의 식당에서 메뉴를 구현한다고 가정해보자.
...
- 위와 같이 `Iterator` 로 분리한 후에 `Waitress` 클래스에서 컬렉션을 받아서 프린트하는 메서드를 작성하면 `main` 에서는 내부 로직을 신경쓰지 않고 모든 항목들에 대해 반복 작업을 수행할 수 있게 된다.
...
- 데이터의 변경이 발생했을 경우 상대 클래스나 객체에 의존하지 않으면서 데이터 변경을 통보하고자 할 떄 유용하다.

### Observer 패턴의 구현

- Observer 패턴을 구현하는 방법에는 여러가지가 있지만, 대부분 `주체(Subject) 인터페이스`와` 옵저버(Observer) 인터페이스`가 들어있는 클래스로 디자인한다.

<img src="https://github.com/dltkd1395/CS-study/blob/main/DesignPattern/image/observer2.png" style="max-width: 100%; display: inline-block;" data-target="animated-image.originalImage">
...
- Observer에서 `update()` 메서드를 통해 주제의 state를 전달받긴 하지만, 실제 state 데이터의 주인은 주체(Subject)에 있다.
- Observer는 데이터가 변경되었을 때 주제에서 데이터를 전달해주기를 기다리는 입장이기 때문에 의존성을 가진다고 할 수 있다.
- 이런 방법을 사용하면 여러 객체에서 동일한 데아터를 제어하도록 하는 것에 비해 더 깔끔한 객체지향 디자인을 만들 수 있다.

### 느슨한 결합(Loose Coupling)의 위력 (= Observser 패턴의 장점)
- 두 객체가 느슨하게 결합되어 있다는 것은, 그 둘이 상호작용을 하긴 하지만 서로에 대해 잘 모르는 것을 의미한다. Observer 패턴에서는 주제와 옵저버가 느슨하게 결합되어 있는 객체 디자인을 제공한다.
-  주제가 옵저버에 대해서 아는 것은 옵저버가 특정 인터페이스(Observer 인터페이스)를 구현한다는 것 뿐이기 떄문이다.

</br></br></br>
- 느슨한 결합의 장점은 다음과 같다.
1. 옵저버를 언제든 새로 추가, 제거할 수 있다.
2. 새로운 형식의 옵저버를 추가하려고 할 때 주제를 전혀 변경할 필요가 없다.
3. 주제와 옵저버를 서로 독립적으로 재사용할 수 있다.
...
**기상 모니터링 애플리케이션 개요**
이 시슽엠은 아래 세 요소로 이루어진다.

<img src="https://github.com/dltkd1395/CS-study/blob/main/DesignPattern/image/observer3.png" style="max-width: 100%; display: inline-block;" data-target="animated-image.originalImage">

1. 기상 스테이션(실제 기상 정보를 수집하는 장비)
2. WeatherData 객체(기상 스테이션으로부터 오는 데이터를 추적하는 객체)
3. 사용자에게 현재 기상 조건을 보여주는 디스플레이
</br></br>
- WeatherData 객체에서는 기상 스테이션 장비 자체로부터 데이터를 가져올 수 있다. 데이터를 가져온 후에는 디스플레이 장비에 세 가지 항목을 표시할 수 있다.
  - 현재 조건(온도, 습도, 압력)
  - 기상 통계
  - 간단한 기상 예보
- 이 화면들은 모두 WeatherData 객체에서 최신 데이터로 업데이트될 때 마다 실시간으로 갱신된다.
...
- Weather Data 클래스가 온도, 습도, 기압을 가져오는 get 메서드를 제공하고, 기상 관측값이 갱신되면 `measurementsChanged()` 메서드를 호출한다.
- 그럼 `measurementsChanged()` 메서드를 3가지 화면을 구현해보자.
...
- get 메서드들을 이용하여 온도, 습도, 기압을 가져오고 각 화면단에 해당 변수들을 전달하여 `update()` 를 하는 방식으로 구현했다.
- 하지만 이 코드에는 아래와 같은 문제가 발생한다.
1. `currentWeatherDisplay.update`, `forecastWeatherDisplay.update`, `staticsticDisplay.update` 와 같은 구체적인 구현에 맞춰서 코딩했기 때문에, 다른 화면을 추가하고자 한다면 `measurementsChanged()` 메서드를 고치지 않고서는 새로운 화면을 추가할 수 없다.
2. 따라서 바뀔 수 있는 부분(각 화면 단)을 캡슐화 해야한다.

### Good Case(Observer 패턴 적용 후)

<img src="https://github.com/dltkd1395/CS-study/blob/main/DesignPattern/image/observer4.png" style="max-width: 100%; display: inline-block;" data-target="animated-image.originalImage">
...
3. 위 인터페이스를 구현한 현`재 날씨, 기상 예보, 기상 통계` 클래스 만들기
...
- State Pattern은 객체의 내부 상태에 따라 객체의 행위를 바꿔야 할 떄, 행위를 객체화 하여 상태를 직접 확인하지 않고 상태 객체에 상태에 따라 다른 행위를 하도록 위임하는 패턴이다.
- 그래서 객체의 상태를 인터페이스로 캡슐화한 뒤, 특정 상태를 클래스로 선언하고 그 상태에서 할 수 있는 행위들을 메서드로 정의하여 사용한다.

<img src="https://github.com/dltkd1395/CS-study/blob/main/DesignPattern/image/state1.png" style="max-width: 100%; display: inline-block;" data-target="animated-image.originalImage">

- Context : 클라이언트 객체를 의미한다. request를 호출하면 state의 handle을 요청한다.
- State : 객체의 행위를 정의한 인터페이스이다.
- ConcreteState : State 인터페이스를 구현한 클래스이다. Context로 부터 전달된 요청을 처리한다. 각 클래스는 각각의 상태에 따라 수행할 handle 메서드를 구현한다.

### State Pattern 사용 예

- ON, OFF 스위치를 가진 형광등 클래스를 구현한다.
- ON을 누르면 불이 켜지고, OFF를 누르면 불이 꺼진다. 불이 켜졌을 떄 ON을 누르거나 불이 꺼졌을 때 OFF를 누르면 동작하지 않는다.
- 사용 전
...
- 하지만, 형광등의 불이 켜졌을 때 ON 버튼을 누르면 수면 모드가 되도록 기능을 추가한다면  코드에는 더 많은 변수와 if문이 필요하다. 그래서 위와 같은 구조라면 기능이 추가될 떄마다 코드가 수정되어야 하고, 유지보수가 힘들어 질것이다. 즉 OCP를 만족하지 않는다.
...
- State Pattern을 적용하면 클라이언트는 현재 상태와 그에 따른 행위를 상태 클래스에 모두 위임한다. 그래서 새로운 기능이 추가될 떄, 상태 클래스를 추가 구현하기도 하면 되므로 OCP를 만족한다.

### 장점
- 하나의 객체에 대한 여러 동작을 구현해야 할 떄 상태 객체만 수정하므로 동작의 추가, 삭제 및 수정이 간단해진다.
- 객체의 상태에 따른 조건문(if/else, switch)이 줄어들어 코드가 간결해지고 가독성이 올라간다.
...
- Strategy Pattern과 State Pattern은 클래스 다이어그램이 똑같다. 하지만 사용 용도에는 차이가 있다.
- Strategy Pattern 실행 중에 객체의 행동을 변경시킬 수 있는 유연성을 제공하는 것이 목적이다. 그래서 상속을 대신해서 구성을 통해 행동을 정의하는 전략 객체를 유연하게 바꿀 수 있게 설계하였다. 그러므로 클라이언트에서 컨텍스트 객체한테 어떤 전략 객체를 사용할 지 지정해야하며, 클라이언트는 전략들을 선택해야하기 때문에 어떤 전략들이 있는지 알고 있어야 한다.
- 하지만, State Pattern은 객체의 상태에 따라 행동을 다르게 하기 위해 객체의 상태 체크에 필요한 코드 내의 조건문들을 없애는 것이 목적이다. 그래서 상태 객체를 만들어서 알고리즘을 캡슐화한 뒤, 내부 상태에 따라 상태 객체의 행동이 바뀌도록 설계하였다. 그러므로 클라이언트에서는 상태 객체에 대해 몰라도 된다.
...
### Visitor
- `객체`(데이터 구조)와 `로직`(알고리즘)을 분리하는 디자인 패턴이다.
...
- 이렇게 객체를 분기할 경우 `1) 특정 객체에 대한 분기`를 빼먹을 수 도 있을 뿐만 아니라, `비슷한 구조가 반복되는 코드 중복`이 발생한다.
- 즉, 1번의 문제를 거의 해결하지 못한 격이다.

### 문제3. Single Dispatch(Dynamic Dispatch) 문제
### Dispatch란?
...
### 예제
1. 2번 예제의 상품 객체와 주문 클래스를 유지하되, 주문 클래스에서 각 상품 타입 별 메서드르 생성한다.
...
- 이번 예제는 2번 예제의 실행 부분인 main() 부분이 동일하다. 언뜻보면 기능해 보일지라도 이번에는 `컴파일 에러` 가 발생하게 된다.
- OrderImpl 클래스 내부의 `pack()` 메서드와 `deliver()` 메서드는 각각 서로 다른 매개변수 타입(`Glass`, `FreshGrocery`)으로 정의하여 오버로딩하고 있다.
...
- 이 문제를 해결하고자 `glass` 와 `freshGrocery` 인스턴스를 `Product` 타입이 아닌 `Glass` 타입과 `FreshGrocery` 타입으로 생성하게 된다면 이는 더 이상 인터페이스를 사용하는 의미가 없는 코드가 되어 버린다.

</br></br></br>

### Visitor Pattern을 이용한 해결방안
...
> - Visitor : 기능이나 로직을 우한 인터페이스 ex) `Order 인터페이스`
